...
export const resolveConnectionRequestWifiBinaryData = (
	...
	const size =
		CONNECTION_REQUEST_INFO_LENGTH + SSID_MAX_LENGTH + PASSWORD_MAX_LENGTH;
	...
	view.setUint32(
		CONNECTION_REQUEST_SIZE_INFO_LENGTH,
		size - CONNECTION_REQUEST_INFO_LENGTH,
		true,
	);
...
export const resolveConnectionRequestFrameBinaryData = (
	...
	colors.forEach((color, index) => {
		view.setUint8(
			offset + index * CONNECTION_REQUEST_FRAME_COLOR_LENGTH,
			color,
		);
	});
...
export const connectionRequestDataToBinaryData = (
	...
	const totalLength = binaryResults.reduce(
		(accumulator, buffer) => accumulator + buffer.length,
		0,
	);
