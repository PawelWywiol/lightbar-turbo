...
const getDeviceFormattedLeds = (device: ConnectedDevice): string =>
	device.info?.data.leds
		? `${device.info?.data.leds} ${MESSAGES.device.leds}`
		: "";

const getDeviceStatusMessage = (device: ConnectedDevice): string =>
	device.info?.message ??
	MESSAGES.connection[
		(device.status?.toLocaleLowerCase() as keyof typeof MESSAGES.connection) ??
			"closed"
	];

const getFormattedFreeDeviceSpace = (
	device: ConnectedDevice,
): string | undefined =>
	device.info?.data.free ? formatBytes(device.info.data.free) : undefined;

const getFormattedTotalDeviceSpace = (
	device: ConnectedDevice,
): string | undefined =>
...
export const ConnectedDeviceInfo = ({
	...
	const deviceSpaceSeparator =
		formattedFreeDeviceSpace && formattedTotalDeviceSpace ? " / " : "";
	...
	return (
		<div className="text-left h-10 grid grid-cols-[auto,1fr,auto] w-full gap-x-4 items-center">
			...
			<span className="text-right text-xs">
				{formattedFreeDeviceSpace} {deviceSpaceSeparator}{" "}
				{formattedTotalDeviceSpace}
			</span>
		</div>
	);
};
