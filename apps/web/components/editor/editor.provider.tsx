...
import {
	EDITOR_INITIAL_RECENT_COLORS_INDEX_MODULO,
	EDITOR_MAX_HISTORY,
} from "./editor.config";
...
const resolveLightnessColorPalette = (
	color: LightColor,
): EditorColorPalette[] => {
	...
	return editorColorPalette.filter(
		(paletteColor) =>
			paletteColor.index % LIGHTS_PALLETTE_HUE_MAX === partIndex,
	);
};
...
export const EditorProvider = ({
	...
	const [lightsScheme, setLightsScheme] =
		useState<LightsSchemeData>(initialSchemeData);
	const [lightsSchemeHistory, setLightsSchemeHistory] = useState<
		LightsScheme[]
	>([]);
	...
	const [recentColors, setRecentColors] =
		useState<EditorColorPalette[]>(initialRecentColors);
	...
	const handleRedo = useCallback(() => {
		...
		if (
			lightsSchemeHistoryIndex < lightsSchemeHistory.length - 1 &&
			nextScheme
		) {
			setLightsSchemeHistoryIndex(lightsSchemeHistoryIndex + 1);
	...
	const handleSave = useCallback(() => {
		dispatchCustomEvent({
			...
			detail: {
				...
				scheme: resolveLightsSchemeColorIndexes(
					lightsScheme.scheme,
					lightsLayout.value,
				),
			},
